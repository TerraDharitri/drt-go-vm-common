...
func TestMultiDCDTNFTTransferParse(t *testing.T) {
	...
	t.Run("MultiNFTTransferWithSCCall", func(t *testing.T) {
		...
		dataField := []byte("MultiDCDTNFTTransfer@000000000000000005001e2a1428dd1e3a5146b3960d9e0f4a50369904ee5483@02@4c4b4452542d616162393130@0d3d@058184103ad80ffb19f7@4c4b46524d2d396431656138@1ecf06@0423fc01830d455ee5510c@656e7465724661726d416e644c6f636b5265776172647350726f7879@00000000000000000500656d0acc53561c5d6f6fd7d7e82bf13247014f615483")
		...
		require.Equal(t, &ResponseParseData{
			...
			Tokens:           []string{"LKDRT-aab910-0d3d", "LKFRM-9d1ea8-1ecf06"},
	...
	t.Run("MultiDCDTNFTTransferWithMOA", func(t *testing.T) {
		moaIdentifierHex := hex.EncodeToString([]byte(vmcommon.MOAIdentifier))
		dataField := []byte(fmt.Sprintf("MultiDCDTNFTTransfer@000000000000000005001e2a1428dd1e3a5146b3960d9e0f4a50369904ee5483@02@4d4949552d61626364@00@01@%s@00@05", moaIdentifierHex))
		res := parser.Parse(dataField, sender, sender, 3)
		rcv, _ := hex.DecodeString("000000000000000005001e2a1428dd1e3a5146b3960d9e0f4a50369904ee5483")
		require.Equal(t, &ResponseParseData{
			Operation:        "MultiDCDTNFTTransfer",
			DCDTValues:       []string{"1", "5"},
			Tokens:           []string{"MIIU-abcd", vmcommon.MOAIdentifier},
			Receivers:        [][]byte{rcv, rcv},
			ReceiversShardID: []uint32{1, 1},
		}, res)
	})
